...
class Chunk:
    ...
    is_proposition: bool = False  # Whether this is a proposition chunk
...
class IndexResult:
    ...
    Contains statistics about the created chunks and propositions.
    """
...
class Scope(BaseModel):
    ...
    tags: Optional[List[str]] = Field(
        None, description="List of tags (e.g., ['journal', 'work'])"
    )
...
class ChatRequest(BaseModel):
    ...
    history: Optional[List[ChatTurn]] = Field(
        None, description="Conversation history (optional, max 5 turns)"
    )
...
class Citation(BaseModel):
    ...
    source_path: str = Field(
        ..., description="Source file path (for frontend compatibility)"
    )
...
class ChatResponse(BaseModel):
    ...
    answer_blocks: List[AnswerBlock] = Field(
        default_factory=list, description="Structured answer blocks for UI rendering"
    )
...
class StatusResponse(BaseModel):
    ...
    index_status: str = Field(
        ..., description="Index status (e.g., 'ready', 'empty', 'error')"
    )
...
class ConfigUpdateRequest(BaseModel):
    ...
    llm_provider: Optional[str] = Field(
        None, description="LLM provider (local or openai)"
    )
    llm_model: Optional[str] = Field(None, description="LLM model name")
    llm_base_url: Optional[str] = Field(None, description="Local LLM endpoint URL")
    embedding_provider: Optional[str] = Field(
        None, description="Embedding provider (local or openai)"
    )
    embedding_model: Optional[str] = Field(None, description="Embedding model name")
    vector_store_path: Optional[str] = Field(
        None, description="Vector store storage path"
    )
    openai_api_key: Optional[str] = Field(None, description="OpenAI API key")
    api_port: Optional[int] = Field(None, description="API server port")
    use_keyring: Optional[bool] = Field(
        None, description="Use OS keychain for API keys"
    )
...
class FeedbackRequest(BaseModel):
    ...
    feedback_type: Literal["positive", "negative"] = Field(
        ..., description="フィードバック種別"
    )
    improvement_request: Optional[str] = Field(
        None, description="改善リクエスト（negativeの場合）"
    )
