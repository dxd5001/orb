...
class ConfigManager:
    ...
    def __init__(self, env_file: Optional[str] = None):
        ...
        self.env_file = (
            Path(env_file).resolve() if env_file else self.project_root / ".env"
        )

        if self.env_file.exists():
            load_dotenv(self.env_file)
        else:
            load_dotenv()  # Fall back to default dotenv behavior
    ...
    def get_config(self, key: str) -> Optional[str]:
        ...
        if key == "VECTOR_STORE_PATH" and value:
            return str(self._resolve_project_path(value))
    ...
    def _resolve_project_path(self, path_value: str) -> Path:
        ...
        Args:
            path_value: Raw path value from configuration
    ...
    def get_llm_config(self) -> Dict[str, Any]:
        ...
        config = {"provider": provider.lower(), "model": model}
        ...
        if provider.lower() == "local":
            ...
            if not api_key:
                raise ValueError(
                    f"{provider.upper()}_API_KEY must be configured for cloud LLM"
                )
    ...
    def get_embedding_config(self) -> Dict[str, Any]:
        ...
        if not provider or not model:
            raise ValueError(
                "EMBEDDING_PROVIDER and EMBEDDING_MODEL must be configured"
            )
        ...
        if provider.lower() == "local":
            ...
            if not api_key:
                raise ValueError(
                    f"{provider.upper()}_API_KEY must be configured for cloud embedding"
                )
            config["api_key"] = api_key
